...
class Board:
    ...
    def castling0(self):
        return self.castling(0, 4, 3, 2)

    def castling7(self):
        return self.castling(7, 4, 5, 6)

    def castling(self, rook_col, king_col, target_rook_col, target_king_col):
        row = 0 if self.current_player_color() == WHITE else 7
        king = self.field[row][king_col]
        rook = self.field[row][rook_col]
        if king is None or rook is None:
            return False
        if rook.char() != 'R' or rook.was_moved():
            return False
        if king.char() != 'K' or king.was_moved():
            return False
        if not (self.field[row][target_rook_col] is None):
            return False
        if not rook.can_move(self, row, rook_col, row, target_rook_col):
            return False
        if not (self.field[row][target_king_col] is None):
            return False
        self.move_piece(row, rook_col, row, target_rook_col)
        self.field[row][king_col] = None
        self.field[row][target_king_col] = king
        king.move()
        rook.move()
        return True
...
# файла (без .py), если этот файл импортирован из другого, и "__main__", если
